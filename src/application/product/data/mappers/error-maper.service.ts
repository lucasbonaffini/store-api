...
export class ErrorMapperService {
  ...
  mapError(
    ...
    if (this.isPrismaError(error)) {
      return this.prismaErrorMapper.mapToDatabaseException(error, operation);
    }
  ...
  mapProductFetchByIdError(error: Error, id: number): Error {
  ...
  mapProductUpdateError(error: Error, id: number): Error {
    return this.mapError(error, `update product with id ${id}`, {
      productId: id,
    });
  }

  mapStockUpdateError(error: Error, id: number): Error {
    return this.mapError(error, `update stock for product with id ${id}`, {
      productId: id,
    });
  }

  mapProductDeleteError(error: Error, id: number): Error {
